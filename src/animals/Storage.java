...
public class Storage {
    ...
    // Read animal data.
    public static Node readAnimalData(String storageType, String languageCode) {
        // Get Jackson object mapper.
        ObjectMapper objectMapper = getObjectMapper(storageType);

        // Does file not exist?
        File file = new File(getAnimalFileName(storageType, languageCode));
        if (!file.exists()) {
            return null;
        }
        ...
        Node rootNode = null;
        try {
            rootNode = objectMapper.readValue(file, Node.class);
        } catch (IOException e) {
    ...
    // Write animal data.
    public static void writeAnimalData(String storageType, Node rootNode, String languageCode) {
        // Get Jackson object mapper.
        ObjectMapper objectMapper = getObjectMapper(storageType);
    ...
    // Get animal file name.
    private static String getAnimalFileName(String storageType, String languageCode) {
        return switch (languageCode) {
            case "en" -> {
                yield ANIMAL_FILE_NAME + "." + storageType;
            }
            default -> {
                yield ANIMAL_FILE_NAME + "_" + languageCode + "." + storageType;
            }
        };
    }
}
